...
{
    public partial class FrmReservaOrigem : Form
    {
        #region Contants
        ...
        private const string m_colEstadoEcf = "EstadoDestino";

        #endregion

        public FrmReservaOrigem()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                for (int i = 1; i <= priGrelhaDocs.Grelha.DataRowCnt; i++)
                {
                    ...
                    if (priGrelhaDocs.GetGRID_GetValorCelula(i, m_colSeleccionado) == 1)
                    {
                        ...
                        objLinhaReserva.Armazem = priGrelhaEncomendas.GetGRID_GetValorCelula(priGrelhaEncomendas.Grelha.ActiveRow, m_colArmazem);
                        objLinhaReserva.Localizacao = priGrelhaEncomendas.GetGRID_GetValorCelula(priGrelhaEncomendas.Grelha.ActiveRow, m_colLocalizacao);
                        objLinhaReserva.Lote = priGrelhaEncomendas.GetGRID_GetValorCelula(priGrelhaEncomendas.Grelha.ActiveRow, m_colLote);

                        objLinhaReserva.IdTipoOrigemOrigem = PriEngine.Engine.Inventario.TiposOrigem.DaIDTipoOrigem(ConstantesPrimavera100.AbreviaturasApl.Compras, ConstantesPrimavera100.Modulos.Compras);
                        objLinhaReserva.DescricaoOrigem = priGrelhaEncomendas.GetGRID_GetValorCelula(priGrelhaEncomendas.Grelha.ActiveRow, m_colDocumento);
                        objLinhaReserva.IdChaveOrigem = priGrelhaEncomendas.GetGRID_GetValorCelula(priGrelhaEncomendas.Grelha.ActiveRow, m_colIdLinha);
                        objLinhaReserva.EstadoOrigem = priGrelhaEncomendas.GetGRID_GetValorCelula(priGrelhaEncomendas.Grelha.ActiveRow, m_colEstadoEcf);


                        objLinhaReserva.IdTipoOrigemDestino = priGrelhaDocs.GetGRID_GetValorCelula(i, m_colIdTipoOrigem);
                        objLinhaReserva.IdChaveDestino = priGrelhaDocs.GetGRID_GetValorCelula(i, m_colIdLinha);
                        objLinhaReserva.DescricaoDestino = priGrelhaDocs.GetGRID_GetValorCelula(i, m_colDocumento);
                        objLinhaReserva.TipoDocDestino = priGrelhaDocs.GetGRID_GetValorCelula(i, m_colTipoDoc);
                        ...
                        objLinhaReserva.ReservadoPor = InvBE100.InvBETipos.EnumReservadoPor.Origem;
                ...
                CmpBEDocumentoCompra objDoc = new CmpBEDocumentoCompra();
                objDoc = PriEngine.Engine.Compras.Documentos.EditaID(strIdEncomenda);

                CmpBELinhaDocumentoCompra objLinha = ((IEnumerable<CmpBELinhaDocumentoCompra>)objDoc.Linhas).FirstOrDefault(x => x.IdLinha == strIdLinha);
                InvBE100.InvBEReserva objReserva = new InvBE100.InvBEReserva();
                objReserva.Linhas = objReservas;
                objLinha.ReservaStock = objReserva;

                PriEngine.Engine.Compras.Documentos.Actualiza(objDoc);
        ...
        private void InicializaColunasGrelhaEncomendas()
        {
            ...
            strTitulo = PriEngine.Platform.Localizacao.DaResString(ConstantesPrimavera100.AbreviaturasApl.Base, 780);
            strCampoBD = PriEngine.Platform.Strings.Formata("@1@ = L.Armazem", m_colArmazem);
            priGrelhaEncomendas.AddColKey(m_colArmazem, UpgradeHelpers.Spread.FpCellType.CellTypeStaticText, strTitulo, 10, true, true, -999999999, 999999999, 2, 50, "", false, false, strCampoBD, true, false, false);

            strTitulo = PriEngine.Platform.Localizacao.DaResString(ConstantesPrimavera100.AbreviaturasApl.Base, 781);
            strCampoBD = PriEngine.Platform.Strings.Formata("@1@ = L.Localizacao", m_colLocalizacao);
            priGrelhaEncomendas.AddColKey(m_colLocalizacao, UpgradeHelpers.Spread.FpCellType.CellTypeStaticText, strTitulo, 10, true, true, -999999999, 999999999, 2, 50, "", false, false, strCampoBD, true, false, false);

            strTitulo = PriEngine.Platform.Localizacao.DaResString(ConstantesPrimavera100.AbreviaturasApl.Base, 782);
            strCampoBD = PriEngine.Platform.Strings.Formata("@1@ = CASE WHEN L.Lote = '@2@' THEN '' ELSE L.LOTE END", m_colLote, ConstantesPrimavera100.Inventario.LotePorDefeito);
            priGrelhaEncomendas.AddColKey(m_colLote, UpgradeHelpers.Spread.FpCellType.CellTypeStaticText, strTitulo, 10, true, true, -999999999, 999999999, 2, 50, "", false, false, strCampoBD, true, false, false);

            strTitulo = "Estado";
            priGrelhaEncomendas.AddColKey(m_colEstadoEcf, UpgradeHelpers.Spread.FpCellType.CellTypeStaticText, strTitulo, 13, true, true, -999999999, 999999999, 2, 50, "", false, false, m_colEstadoEcf, true, true);
        ...
        private void CarregaDestinosPossiveis(string Artigo)
        {
            ...
            var objListaDocs = funcoesComunsBS.Documentos.LstReservasDestinosPossiveis(Artigo, true, true, true
                                                                                    , priGrelhaDocs.DaCamposBDSelect().Replace("''", "'")
                                                                                    , new DateTime(DateTime.Today.Year, 1, 1), new DateTime(DateTime.Today.Year, 12, 31), "", strTabelaTemp
                                                                                    , ""
                                                                                    , "");
            //priGrelhaDocs.DataBind(objListaDocs1);
        }

        private void CarregaListaEncomendas()
        {
            string strSql = "SELECT " + priGrelhaEncomendas.DaCamposBDSelect().Replace("''", "'") + " FROM LinhasCompras L" + Environment.NewLine;
            strSql += "INNER JOIN CabecCompras C ON C.Id = L.IdCabecCompras" + Environment.NewLine;
            strSql += "INNER JOIN CabecComprasStatus CS ON CS.IdCabecCompras = C.Id" + Environment.NewLine;
            strSql += "INNER JOIN LinhasComprasStatus LS ON LS.IdLinhasCompras = L.Id" + Environment.NewLine;
            strSql += "INNER JOIN DocumentosCompra D ON D.Documento = C.TipoDoc" + Environment.NewLine;
            strSql += "WHERE D.TipoDocumento = 2 AND D.LigaStocks = 1 AND CS.Fechado = 0 AND cs.Anulado = 0 AND" + Environment.NewLine;
            strSql += "      LS.Fechado = 0 AND Ls.QuantTrans = 0" + Environment.NewLine;
            strSql += "      AND Year(C.DataDoc) = 2019 AND L.TipoLinha NOT IN (60,65)" + Environment.NewLine;
            strSql += "ORDER BY C.DataDoc DESC" + Environment.NewLine;
            ...
            {
                CarregaDestinosPossiveis(priGrelhaEncomendas.GetGRID_GetValorCelula(1, m_colArtigo));
            }
        }
        ...
        {
            CarregaDestinosPossiveis(priGrelhaEncomendas.GetGRID_GetValorCelula(e.NewRow, m_colArtigo));
        }
    }
}
